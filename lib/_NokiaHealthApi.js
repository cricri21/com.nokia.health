...
/*
	API Documentation: https://account.health.nokia.com/doc/oauth2/user
*/
...
class NokiaHealthApi extends events.EventEmitter {
	...
	constructor() {
		...
		this._webhooksUri = `https://webhooks.athom.com/webhook/${this._webhooksId}?r=${Math.random()}`;
		this._token = null;
	}
	...
	*/
	async _call( method, path, qs, data, isRefreshed ) {
		...
		let qsObj = {};
		qsObj.access_token = this._token.access_token;
		for( let key in qs ) {
			let value = qs[key];
		...
		let qsString = querystring.stringify(qsObj);

		let url = `${this._apiUrl}${path}?${qsString}`;
		...
		return rp({
			...
			if( !isRefreshed && result && result.status === 401 )
				return this.refreshOAuth2Token().then( token => {
					...
					return this._call( method, path, qs, data, true );
				});
	...
	async createSubscription( userId ) {
		...
		return Promise.all(
			[].concat([
				...
				[ 1, 4, 16, 44 ].map(activityId => {
					return this._get('/notify', {
						action: 'subscribe',
						callbackurl: this._webhooksUri,
						appli: activityId
					})
				})
			])
	...
	async deleteSubscription() {
		return Promise.all([
			...
			this._get('/notify', {
				action: 'revoke',
				callbackurl: this._webhooksUri,
				appli: 45 // hardcoded 'environment'
			})
		]);
	}
	...
	*/
	...
	async _unregisterWebhook() {
		...
		this._webhook.removeAllListeners();
		return this._webhook.unregister();
	}

	_onWebhookMessage( message ) {
		...
		this.getMeasurement({
			...
			this.emit('measurements', measurements);
		}).catch( err => {
			console.error(err);
		})
	}
	...
	static parseMeasurementResult( result ) {
		...
		for( let i = 0; i < result.measuregrps.length; i++ ) {
			...
			for( let j = 0; j < grp.measures.length; j++ ) {
				...
				let type = measurementTypeMap[ measure.type ];
